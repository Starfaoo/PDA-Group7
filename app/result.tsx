...
export default function ResultScreen() {
  ...
  // Data from the scan with safe parsing
  const {
    imageUri,
    disease,
    confidence,
  ...
  // Safely parse severity level (1-5)
  const severityLevel = severity
    ? Math.min(5, Math.max(1, parseInt(severity as string)))
    : 3;

  // Safely parse factors with error handling
  let factorsData = {
    humidity: "Unknown",
    sunlight: "Unknown",
    airflow: "Unknown",
  ...
  if (factors) {
    try {
      const parsed = JSON.parse(factors as string);
      factorsData = {
        humidity: parsed?.humidity || "Unknown",
        sunlight: parsed?.sunlight || "Unknown",
        airflow: parsed?.airflow || "Unknown",
      };
    } catch (e) {
      console.warn("Failed to parse factors data:", e);
    }
  }
      ...
      >
        ...
        <View style={styles.body}>
          ...
          <View style={styles.titleRow}>
            ...
            <TouchableOpacity
              onPress={() => Alert.alert("Info", description as string)}
            >
              <Ionicons
              ...
              {[1, 2, 3, 4, 5].map((level) => (
                <View
                  ...
                  style={[
                    ...
                    {
                      backgroundColor:
                        level <= severityLevel ? "#FFC107" : "#E0E0E0",
                    },
                  ]}
                />
              ))}
          ...
          <View style={styles.factorsGrid}>
            {factorsList.map((item, index) => (
              <View
                key={index}
                style={[styles.factorCard, { backgroundColor: item.color }]}
              >
                {index === 2 ? (
                  <MaterialCommunityIcons
                    name="weather-windy"
